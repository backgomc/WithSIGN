...
- 대량 전송 검색 기능 버그 - 전체 선택 후 특정 사용자 제외시 체크한 기록 날라감
...
# 2.1.2
- [*] 최초 비밀번호 변경 시 토큰 검증로직 제외 / 설정 비밀번호 변경 부분은 분리 필요
- [*] 세션 로그아웃과 동시에 세션만료알림 중복 알림 현상 발생  (admin alert 주석처리)
- [*] withpdf 0.7.1 update (텍스트 컴포넌트 작을때 입력 안되는 문제)

# 2.1.3
- [*] 세션 로그아웃과 동시에 세션만료알림 중복 알림 현상 발생 (client alert 주석처리)

# 2.1.4
- [*] withpdf 업데이트 (기존 템플릿에 텍스트 컴포넌트 입력이 안되는 문제) 

# 2.1.5
- [*] 템플릿 첨부파일 등록하는 경우가 있으므로 일반요청시 1단계로 보냄 (ASIS-2단계) 

# 2.1.6 server only
- [*] 첨부 파일이 여러개인 경우 2번째 파일이 첫 번째 파일을 덮어 쓰는 경우 발생

# 2.1.7 : 2024/01/12 배포
- client
  - [*] checkbox 기능 개선 (withpdf 0.7.3 으로 업데이트)
- server
  - [*] 멀티 다운로드 안되는 문제 fixed (파일 경로가 긴 경우 오류 발생)

# 2.1.8 (client, server all)
 - [*] withpdf 0.8 update
    - x,y 좌표 기능
    - 이미지 컴포넌트 지원
    - 서명 팝업 가끔 서명 안되는 문제 해결
 - [*] 신청서 이미지 클릭 시 동작하도록 변경
 - [*] pdf 사이즈 변경 시 전체 스크롤 반영
 - [*] 수신자 워킹 그룹 처리 (신청서 only)
 - [*] 템플릿 파일 업데이트 기능 
...
- [ ] 링크(바로) 서명 기능 개발 
- [*] 대용량 대비 목록 통신 최소화
- [ ] 신청서의 경우 동일 단계에 수신자가 여러명인 경우 그룹핑 처리 (한명만 수신해도 완료 처리)
  - [ ] 수신 완료 처리 시 동차에 수신자가 있는 경우 완료처리 하기
  - [ ] 신청서 화면에서 동차에 사용자의 경우 외 몇 명 출력하기
  - [ ] 문서 상세 조회 시 수신완료시간이 없는 경우 비워놓거나 타담당자 처리완료 
- [ ] 신청서 다른사람이 작성한 문서 수정 가능하게 하기 
- [ ] 템플릿 문서만 교체 (컴포넌트 유지)
...
- docType: L (B, G)
- 문서함을 같이 사용할 것인가?
  - 같이 사용하하는 경우 문서 조회 쿼리시 ... populate users 
- user 를 같이 사용할 것인가?
  - QR서명 입력시 사용자 테이블에 사용자를 넣어줘야하는데 ...
  - 분리할 경우 사용자와 이메일사용자가 같이 서명하는 경우가 문제인데 ...
  - 회원가입을 안한상태에서 user 에 이름,이메일정보를 저장하는게 맞는가? NO
  - users 는 만들지 않는다. YES
  - 참여자정보는 문서에 포함한다. YES

  => 별도로 구성하되 문서함에 혹시 포함하여 향후 출력하는 경우 오류가 나지않도록 개발한다.
  => documents 는 같이 구성한다.
  => users 는 분리한다.
