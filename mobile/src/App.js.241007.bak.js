      ...
      if (response.data.success) {
        ...
        if (localStorage.getItem('__rToken__')) {
          ...
          axios.post('api/users/refresh', null, config).then(response => {
            if (response.data.success) {
              console.log('[checkUser-refresh] response.data.user : ', response.data.user)
              pageSwitch(response.data.user)
            } else {
              dispatch(setUser(null));
              console.log('[CSI] login - refresh : ', response.data)
          ...
          dispatch(setUser(null));
          console.log('[CSI] login - __rToken__ : ', localStorage.getItem('__rToken__'));
          navigate('/login',{ state : { dId : docId}})
        }
      }
    ...
    axios.post('/api/document/document', { docId : docId }).then(function (response) {
      ...
      if (response.data.success) {
        if (response.data.document.canceled === true){
          ...
          if (response.data.document.usersTodo.includes(user._id)){
